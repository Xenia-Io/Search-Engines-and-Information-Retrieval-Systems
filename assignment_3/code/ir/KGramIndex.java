...
public class KGramIndex {
    ...
    public List<KGramPostingsEntry> intersect(List<KGramPostingsEntry> p1, List<KGramPostingsEntry> p2) {
        ...
        while (i < p1.size() && j < p2.size()) {
            if (p1.get(i).tokenID == p2.get(j).tokenID) {
                ...
                KGramPostingsEntry kpe = new KGramPostingsEntry(p1.get(i).tokenID);
    ...
    private void addGram(int idx, String tkn) {
        if (index.get(tkn) == null) {
            List<KGramPostingsEntry> kgl = new ArrayList<KGramPostingsEntry>();
            kgl.add(new KGramPostingsEntry(idx));
            index.put(tkn, kgl);
        } else {
            int last = index.get(tkn).size() - 1;
            if (index.get(tkn).get(last).tokenID != idx) {
                index.get(tkn).add(new KGramPostingsEntry(idx));
            }
        }
